...
class TestKeo(unittest.TestCase):
    ...
    def _run_test(self, filename, mu, control_values):
        ...
        KK = keo - keo.H
        self.assertAlmostEqual(0.0, KK.sum(), delta=tol)

        # Check the matrix sum.
        self.assertAlmostEqual(control_values["sum real"], keo.sum(), delta=tol)
        ...
        self.assertAlmostEqual(control_values["norm 1"], numpy.max(K.sum(0)), delta=tol)
        return
    ...
    def test_rectanglesmall(self):
        ...
        mu = 1.0e-2
        control_values = {
            "sum real": 0.0063121712308067401,
    ...
    def test_pacman(self):
        filename = os.path.join(self.this_path, "pacman.e")
        mu = 1.0e-2
        control_values = {"sum real": 0.37044264296585938, "norm 1": 10.000520856079092}
        self._run_test(filename, mu, control_values)
    ...
    def test_cubesmall(self):
        ...
        mu = 1.0e-2
        control_values = {
            "sum real": 8.3541623155714007e-05,
    ...
    def test_brick(self):
        filename = os.path.join(self.this_path, "brick-w-hole.e")
        mu = 1.0e-2
        control_values = {"sum real": 0.16763276012920181, "norm 1": 15.131119904340618}
        self._run_test(filename, mu, control_values)
