...
def _compute_eigenvalues(
    operator_type, eigenvalue_type, num_eigenvalues, v0, psi, modeleval, mu, g
):
    if operator_type == "k":
        ...
        A = LinearOperator(
            (num_unknowns, num_unknowns), _apply_prec_jacobian, dtype=complex
        )
    else:
        raise ValueError("Unknown operator '%s'." % operator_type)

    print(
        "Compute the %d %s eigenvalues of %s..."
...
def _complex2real(op):
    ...
    return LinearOperator(
        (2 * op.shape[0], 2 * op.shape[1]), _jacobian_wrap_apply, dtype=float
    )
...
def _complex_with_bordering2real(op):
    ...
    n = op.shape[0] - 1
    N = 2 * n + 1
    return LinearOperator((N, N), _jacobian_wrap_apply, dtype=float)
...
def _plot_eigenvalue_series(x, eigenvals_list):
    ...
    for k, eigenvalues in enumerate(eigenvals_list[1:]):  # skip the first
        ...
        reordered_eigenvalues[:, k + 1] = _permutation_match(
            eigenvalues, eigenvals_extrapolation
        )
        # linear extrapolation
        if k + 2 < len(x):
            eigenvals_extrapolation = _linear_extrapolation(
                x[k],
                x[k + 1],
...
def _parse_input_arguments():
    ...
    parser.add_argument(
        "--mu", "-m", dest="mu", type=float, help="magnetic vector potential multiplier"
    )
