...
def _main():
    ...
    for run in timings["tests"]:
        ...
        if run["target"] == "jacobian":
            TJ = min_time
        if (
            run["target"] == newton_data[0]["preconditioner type"]
        ):  # peek at first newton run
            TMinv = min_time
    ...
    assert (
        vanilla_newton_data["preconditioner type"]
        == newton_data[0]["preconditioner type"]
    )
    ...
    for step in newton_steps:
        ...
        for newton_datum in newton_data:
            if step < len(newton_datum["Newton results"]) - 1:
                num_vanilla_steps = (
                    len(vanilla_newton_data["Newton results"][step]["relresvec"]) - 1
                )
                ...
                if num_defl_vecs > 0:
                    ...
                    y.append(
                        Toverall(num_steps, num_defl_vecs)
                        / Toverall(num_vanilla_steps, 0)
                    )

        pp.plot(
            x,
            y,
            color=str(1.0 - float(step + 1) / len(newton_steps)),
            label="step %d" % step,
        )
