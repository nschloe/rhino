...
def _run_different_meshes(modeleval, precs):
    ...
    matplotlib2tikz.save(
        "meshrun.tikz", figurewidth="\\figurewidth", figureheight="\\figureheight"
    )
    pp.show()
    return
...
def _run_preconditioners(linear_operator, rhs, x0, preconditioners):
    ...
    for prec in preconditioners:
        ...
        start_time = time.clock()
        sol, info, relresvec = nm.minres_wrap(
            linear_operator,
            rhs,
            x0=x0,
            tol=tol,
...
def _plot_relresvecs(test_preconditioners, relresvecs):
    # plot them all
    for prec in test_preconditioners:
        pp.semilogy(relresvecs[prec["name"]], label=prec["name"])
...
def _plot_l2_condition_numbers(model_evaluator):
    ...
    mu_max = 5.0
    steps = 2
    mus = np.arange(steps, dtype=float) / (steps - 1) * (mu_max - mu_min) + mu_min
    ...
    for mu in mus:
        ...
        small_eigenval = eigs(
            model_evaluator._keo, k=1, sigma=None, which="SM", return_eigenvectors=False
        )
